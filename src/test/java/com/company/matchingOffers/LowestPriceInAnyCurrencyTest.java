...
class LowestPriceInAnyCurrencyTest {
    private Matchable matcher = new LowestPriceInAnyCurrency();
    ...
    public void should_return_lowest_value_matching_currency_from_buyer_offers() throws NoSuchItemException {
        Money expected = new Money(new BigDecimal(5), Currency.USD);
        buyOffer.getPrices().add(new Money(new BigDecimal(29), Currency.PLN));
    ...
    public void should_throw_NoSuchItemException_if_no_match_found() {
        buyOffer.getPrices().add(new Money(new BigDecimal(16), Currency.PLN));
